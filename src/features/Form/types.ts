...
export type FormReducer = <FormName extends string | number, T>(
  ...
  action: SetFieldValueAction<T, FormName> | SetFieldErrorAction<T> | ClearFieldErrorAction<T>,
) => FormContextState<FormName, T>;

export type UseFormActions<FormName, T> = {
  setFieldValueAction: (formName: FormName, field: FieldName<T>, value: FieldValue) => void;
